...
Ny = 10  #Number of unit cell in y-direction 
Dim = structure.struc(Nx,Ny)

Hopping = Strain.strain(ex, ey, ez, Hopping)


ulist =  np.arange(0.8,0.9,0.1)
for u in ulist:
    U = u
    ################################################
    #######  Call Hamiltonian  ########
    ...
    initial = 'AF' 
    if initial=='Random':
        Density_up = np.random.rand(N)
        Density_dn = np.random.rand(N)  
    
    if initial=='AF':
        Density_up = np.empty(N)
        Density_dn = np.empty(N)
        Density_up[::2]  = 1
        Density_up[1::2] = 0
        Density_dn[::2]  = 0
        Density_dn[1::2] = 1
    ...
    if initial=='FM':
        Density_up = np.ones(N)
        Density_dn = np.zeros(N)
        

    beta = 1e5
    itr = 0
    changeUp, changeDn = 1, 1
    ...
    np.savetxt('Band_up_U_'+str(u)+'.txt', EnergyBand_u)
"""/////////////////////////////////////////////////"""
"""/////////////////////////////////////////////////"""
...
rectangle = patches.Rectangle((-pi/b,0),         # (x,y)
                       2*pi/b,          # width
                       min(abs(EnergyBand_u[:,int(NN/2)])),          # height
                       #fill=False,
                       edgecolor='None',
                       facecolor='yellow',
                       alpha = 1.0)
...
plt.plot(list(range(1,N+1)),Sz,'or')
plt.plot(list(range(1,N+1)),Sz,'-k')
