...
for U in Urange:
    ...
    #######         initial value           ########
    ################################################ 
    Density_up   = np.array([0.5, 0.0, 0.5, 0.0])
    Density_down = np.array([0.0, 0.5, 0.0, 0.5])
...
#    E_GState.append(((Eu+Ed)/mesh**2)-U*np.dot(Density_up,Density_down))
    ...
    Sz_c=np.subtract(Density_up[2],Density_down[2])/2; 
    Magnetization_c.append(Sz_c)
    
    Sz_d=np.subtract(Density_up[3],Density_down[3])/2; 
    Magnetization_d.append(Sz_d)
...
plt.plot(Urange,Magnetization_c,':g',label='$M_c$')
plt.plot(Urange,Magnetization_d,'-or',label='$M_d$')
plt.xlabel('U(eV)')
