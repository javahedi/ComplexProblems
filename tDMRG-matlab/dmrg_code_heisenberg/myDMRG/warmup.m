...
for i=istart:ti-1
   ...
   [psi,Es]=super_block(i,opLe,opRe,0); % psi is the target state
   for j=1:Nop
     SopL{i+1,j}=opLe{j};
     SopR{i+1,j}=opRe{j};
   end
...
for i=ti:iend
   ...
   [psi,Es]=super_block(i,opLe,opRe,0); % psi is the target state
   Eg(i+1-ti)=Es;
   OL=den_mat(psi,i,-1);   % forms reduced density matrix and
   update(i,OL,opLe,-1);
   OR=den_mat(psi,i,+1);   % returns the transformation matrix
   update(i,OR,opRe,+1);
