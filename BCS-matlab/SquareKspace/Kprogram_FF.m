...
clear all
%format long
global Del TT DD HBdG Nx Ny N mu t hh kx ky Temp U0 DelHelicity
global Density Pol Ek UUk alpha GGk QQ DensityHelicity DDH UTT UTTb
global band1 band2 band3 band4

tic

Nx=31;                   % number of mesh over FBZ
Ny=31;
N=Nx*Ny;
...
alpha=2;             % Spin-Orbit intersction
t=1;                    % hopping parameter
U0=-4;
Del=ones(Nx,Ny);        %Initial guess for gap order parameter
DelHelicity=ones(Nx,Ny);
Density=zeros(2,Nx,Ny);
DensityHelicity=zeros(2,Nx,Ny);
Pol=zeros(3,Nx,Ny);
hh(1:1)=0.0;
