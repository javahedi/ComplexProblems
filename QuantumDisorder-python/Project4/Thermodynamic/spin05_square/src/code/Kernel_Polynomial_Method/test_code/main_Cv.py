...
########################################
########################################
ham = get_hamiltonian_sparse(L=12,J1=1.0,J2=1.0,hx=0.0,hz=0.01)
...
T       = np.linspace(0.1,5,100) # temperature vector
...
Cv = []
f1 = open('thermodynamic_KPM.txt','w')
...
for bt in beta:
    ...
    f1.flush()
    cte+=1
    print(f'loop {cte} & time elapsed {time.time()-start}')
f1.close()
