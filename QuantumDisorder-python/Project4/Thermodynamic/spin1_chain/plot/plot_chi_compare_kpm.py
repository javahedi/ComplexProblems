...
for j in range(100,900,100):
    ...
    ax[0].plot(kpmL8[:,0] , kpmL8[:,5]/8 ,label=r'$\mathcal{N}='+str(j)+'$', lw=1.5)
    ax0.loglog(kpmL8[:,0] , kpmL8[:,5]/8 , label=r'$\mathcal{N}='+str(j)+'$', lw=1.5)

    kpmL10 = np.loadtxt(f'{pathKPM}/thermodynamic_KPM_L10_K0.5_NumberMoment{j}.txt',skiprows=2)

    ax[1].plot(kpmL10[:,0] , kpmL10[:,5]/10 , label=r'$\mathcal{N}='+str(j)+'$', lw=1.5)
    ax1.loglog(kpmL10[:,0] , kpmL10[:,5]/10 , label=r'$\mathcal{N}='+str(j)+'$', lw=1.5)
...
ax0.set_ylabel(r'$\chi(T)/L$',fontsize=12)
ax1.set_ylabel(r'$\chi(T)/L$',fontsize=12)
...
plt.savefig('chi_compare_kmp.pdf', bbox_inches='tight')
plt.show()
