...
def run(L, val=0, projection=True):
    ...
    if full_diagonalization:
        ...
        #np.savetxt(f'energy_L{L}.txt',eigs)
...
if __name__ == "__main__":
    ...
    L = 12
    eigs = np.array([])
    for val in np.arange(0,L+1,2):
        start = time.time()
        eigs = np.concatenate((eigs,run(L,val)))
        eigs = np.concatenate((eigs,-1.0 * eigs))
        print(f"Sz : {val}, time : {time.time()- start}")
    np.savetxt(f"energy_L{L}_subspace.txt",np.sort(eigs))
    
    #
    Temp = np.linspace(0.1,5,100)
    Cv = []
    f1 = open("thermodynamic_subspace.txt","w")
