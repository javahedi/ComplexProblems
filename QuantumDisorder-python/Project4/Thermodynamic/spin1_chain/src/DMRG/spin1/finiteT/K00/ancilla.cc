...
main(int argc, char* argv[])
    ...
    */
    ...
    std::ofstream enf("en_K0.0_cutoffE-10_Method_DensityMatrix.dat");
    std::ofstream susf("sus_K0.0_cutoffE-10_Method_DensityMatrix.dat");
    for(int tt = 1; tt <= nt; ++tt)
        ...
        */
        ...
        SzSz(tt-1) = (SziSzj*bb/3.)/N;
        */


        printfln("\nEnergy/N %.4f %.20f, Sus/N %.20f",bb,en/N,Sus(tt-1));
        println();
        enf  << format("%.14f %.14f %.14f\n",Betas(tt-1), 1.0/Betas(tt-1), En(tt-1));
        susf << format("%.14f %.14f %.14f\n",Betas(tt-1), 1.0/Betas(tt-1), Sus(tt-1));
        }
