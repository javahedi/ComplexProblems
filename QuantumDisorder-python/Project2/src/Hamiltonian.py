...
@jit(nopython=True)
def get_hamiltonian_sparse(L, N, J0, hz, site_index,alpha,bc='obc'):
    '''
    ...
    #site_index = [13,20,41,42,50,66,73,76,85,97,100,110]
    ...
    for k in range(1,N-1):
        ...
        for i in range(Nmax):
             #print(k,i,np.mod(i+k,N))
             xx_bonds.append((i,np.mod(i+k,N)))
    ...
    for state in range(hilbertspace_dimension(N)):
        ...
        for bond in xx_bonds:           
            n,m=bond[0],bond[1]
            Zmn = site_index[n]-site_index[m]
            J = J0/(np.sin(Zmn*np.pi/L)**alpha)
