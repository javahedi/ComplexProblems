...
#        time_evolution
###############################
def time_evolution(l,G,H,BC,delta,numsteps,swape):
    L = len(G)
    d = G[0].shape[0]
    ...
    disc = 0.0; temp=0.0
    l, G, tem = evolve_bonds(l,G,expH2,evenbonds,swape); disc += temp

    #for num in range(numsteps-1):
    #    l, G, tem = evolve_bonds(l,G,expH1,oddbonds ,swape); disc += temp
    #    l, G, tem = evolve_bonds(l,G,expH1,evenbonds,swape); disc += temp
...
#        evolve_bonds
###############################
def evolve_bonds(l,G,expH,bonds,swape):
    L    = len(G)
    disc = 0.0
    for pos in bonds:
        posM  = np.mod(pos-1,L)
        posP  = np.mod(pos+1,L)
        if swape:  # swape forward
           G[pos] = G[posP]
        
        G[posM], l[pos], G[pos], temp = evolve_single_bond(l[posM], G[posM], l[pos], G[pos], l[posP],expH)
        
        if swape:  # swape backward
           G[pos] = G[posP]
