...
main()
    ...
    //auto sites = Fermion(N, {"ConserveNf",false});
    auto sites = Z3(N,{"ConserveQNs=",false});

    //Initial Hamiltonian before Quench
    auto ampo0 = AutoMPO(sites);
    for(int j = 1; j <N; ++j)
        {
	      ampo0 += f0,"Sig"   ,j,"SigDag",j+1;
          ampo0 += f0,"SigDag",j,"Sig"   ,j+1;
          ampo0 += J ,"Tau"   ,j;
          ampo0 += J ,"TauDag",j;
        }
    ampo0 += J,"Tau"   ,N;
    ...
    sweeps.maxdim() = 10,50,100,200,500;
    sweeps.cutoff() = 1E-10;
    ...
    for(int j = 1; j <N; ++j)
        {
          ampo += f1,"Sig"   ,j,"SigDag",j+1;
          ampo += f1,"SigDag",j,"Sig"   ,j+1;
          ampo += J ,"Tau"   ,j;
          ampo += J ,"TauDag",j;
        }
    ampo += J,"Tau"   ,N;
    ...
    auto tau = 0.05; //Time step
    //Approx. with error O(tau^4)
    ...
    for(int n = 0; n <= nt; ++n) // loop over time
        ...
        OUT <<n*tau<<"        "<<RP<<"    "<<RF<<"    "<<endl;
        printfln("Tim : %.2f, RP :  %.8f,  RF : %.8f",n*tau,RP,RF);
