...
def initialMat(L,d,chiMax,bc='OBC'):
    ...
    for pos in range(L):
        if pos%2==0:
            ...
            # auxilliary ==> Q
            G[pos][0,1,0] =  1.0/np.sqrt(2)
            G[pos][1,0,0] =  1.0/np.sqrt(2)
        l[pos][  :  ] =  1.0
...
#        Creating MPS
###############################
def initialMat_mix(L,d,chiMax,bc='OBC'):
    G = []  # Gamma
    l = []  # Lambda
    chi = np.zeros(L+1, dtype=np.int32)
...
###############################################
def hamiltinian(Jxy,Jz,hz,hx,mixed=False):
    ...
    if mixed:
       ...
       """
       # first neigbour
       #H  = Jxy * np.real(np.kron(np.kron(np.kron(sx,I1),I1),I1) + np.kron(np.kron(np.kron(sy,I1),I1),I1))
       #H += Jz  *         np.kron(np.kron(np.kron(sz,I1),I1),I1)
              
       # secend neigbour
       H  = Jxy * np.real(np.kron(np.kron(np.kron(sx,I1),sx),I1) +  np.kron(np.kron(np.kron(sy,I1),sy),I1))
       H += Jz  *         np.kron(np.kron(np.kron(sz,I1),sz),I1)
       
       #H  = Jxy * np.real(np.kron(np.kron(np.kron(I1,sx),I1),sx) +  np.kron(np.kron(np.kron(I1,sy),I1),sy))
       #H += Jz  *         np.kron(np.kron(np.kron(I1,sz),I1),sz)
       
       # onsite-term
       H += hz  *         np.kron(np.kron(np.kron(sz,I1),I1),I1)
       H += hx  *         np.kron(np.kron(np.kron(sx,I1),I1),I1)
       """
...
########################################
#       mixing MPS physical with auxilliary
########################################
def lG_AABB(l, G):
    ...
    for pos in range(0,len(A),2):
        temp = np.tensordot( A[pos], A[pos+1], axes=(2,1) )
        
        chi0  = A[pos].shape[1]
        chi1  = A[pos].shape[2]
        chi2  = A[pos+1].shape[2]
        temp = np.reshape  ( temp,  (d*d, chi0, chi2)  )
        AA.append( temp )
        
        temp = np.tensordot( B[pos], B[pos+1], axes=(2,1) )
        chi0  = B[pos].shape[1]
        chi1  = B[pos].shape[2]
        chi2  = B[pos+1].shape[2]
        temp = np.reshape  ( temp,  (d*d, chi0, chi2)  )
        BB.append( temp )
...
def MPS_Vidal(M):
    ...
    for pos in range(L):
        ...
        chi1 = M[pos].shape[2]
        if pos == 0 :
            A        = np.reshape   ( M[pos], (d*chi0,chi1)   )
            U, S, V  = svd          ( A     , full_matrices=0 )
            U        = np.reshape   ( U     , (d,chi0,chi1)   )
            G[pos]   = U
        else :
            SV       = np.tensordot ( np.diag(S)                 , V              , axes=(1,0) )
            A        = np.tensordot ( SV                         , M[pos]         , axes=(1,1) )
            A        = np.transpose ( A                          , (1,0,2)                     )
            A        = np.reshape   ( A                          , (d*chi0,chi1)               )
            U, S, V  = svd          ( A                          , full_matrices=0             )
            U        = np.reshape   ( U                          , (d,chi0,chi1)               )
...
########################################
#    Bring the right "B" canonical to left "A"
#    canonical form to have the Vidal form "G,l"
########################################
...
def evolve_single_bond(l0, G0, l1, G1, l2, expH):
    ...
    l0inv = inverse(l0)#1.0/(l0+1e-20)
    l2inv = inverse(l2)#1.0/(l2+1e-20)
