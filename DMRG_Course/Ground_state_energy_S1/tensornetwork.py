...
def hamiltinian(Jxy,Jz,hz,hx,mixed=False):
    ...
    if mixed:
       d  = 4
       # first neigbour
       H  = Jxy * np.real(np.kron(np.kron(np.kron(sx,sx),I1),I1) + np.kron(np.kron(np.kron(sy,sy),I1),I1))
       H += Jz  *         np.kron(np.kron(np.kron(sz,sz),I1),I1)
       
       H  = Jxy * np.real(np.kron(np.kron(np.kron(I1,sx),sx),I1) + np.kron(np.kron(np.kron(I1,sy),sy),I1))
       H += Jz  *         np.kron(np.kron(np.kron(I1,sz),sz),I1)
       ...
       H += hz  *         np.kron(np.kron(np.kron(I1,sz),I1),I1)
       H += hx  *         np.kron(np.kron(np.kron(sx,I1),I1),I1)
       H += hx  *         np.kron(np.kron(np.kron(I1,sx),I1),I1)
